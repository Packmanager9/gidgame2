...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function setUp(canvas_pass, style = "#000000") {
        ...
        window.addEventListener('pointerdown', e => {
            ...
            if (keysPressed['g']) {
                potjizya.growing = 1
                potjizya.pointer.x = TIP_engine.x
                potjizya.pointer.y = TIP_engine.y
            }
            if (keysPressed['s']) {
                potjizya.growing = -1
                potjizya.pointer.x = TIP_engine.x
            ...
            //     }
            // }
            // if(button.isPointInside(TIP_engine)){
            //     for(let t = 0;t<buttons.length;t++){
            //         buttons[t].y += 21
            //     }
            //     buttons.unshift(new Rectangle(600, 100, 100, 20, getRandomLightColor()))
            // }
            // for (let t = 0; t < balls.length; t++) {
            //     if (balls[t].body.isPointInside(TIP_engine)) {
            //         linkster.add(balls[t].body, balls[t].type)
            //         break
            //     }
            // }
    ...
    //     constructor(x,y){
    //         this.angle = 0
    //         this.anglex = Math.PI*.5
    //         this.dis = 15
    //         this.centroid = new Point(x,y)
    //         this.top = new Circle(this.centroid.x+(Math.cos(this.angle)*this.dis), this.centroid.y+(Math.sin(this.angle)*this.dis), 9, "white")
    //         this.bot = new Circle(this.centroid.x-(Math.cos(this.angle)*this.dis), this.centroid.y-(Math.sin(this.angle)*this.dis), 9, "white")
    //     }
    //     draw(){
    //         canvas_context.imageSmoothingEnabled = false
    //         this.angle += .01
    //         this.anglex -= .03
    //         this.top = new Circle(this.centroid.x+(Math.cos(this.angle)*this.dis), this.centroid.y+(Math.sin(this.angle)*this.dis), 9, "orange")
    //         this.bot = new Circle(this.centroid.x-(Math.cos(this.angle)*this.dis), this.centroid.y-(Math.sin(this.angle)*this.dis), 9, "orange")


    //         this.botx = new Circle(this.bot.x+(Math.cos(this.anglex)*this.bot.radius), this.bot.y+(Math.sin(this.anglex)*this.bot.radius), 3, "#00ffff")
    //         this.boty = new Circle(this.bot.x-(Math.cos(this.anglex)*this.bot.radius), this.bot.y-(Math.sin(this.anglex)*this.bot.radius), 3, "#00ffff")
    //         this.topx = new Circle(this.top.x+(Math.cos(this.anglex)*this.top.radius), this.top.y+(Math.sin(this.anglex)*this.top.radius), 3, "#00ffff")
    //         this.topy = new Circle(this.top.x-(Math.cos(this.anglex)*this.top.radius), this.top.y-(Math.sin(this.anglex)*this.top.radius), 3, "#00ffff")

    //         this.botx2 = new Circle(this.bot.x+(Math.cos(this.anglex-(Math.PI*.5))*this.bot.radius), this.bot.y+(Math.sin(this.anglex-(Math.PI*.5))*this.bot.radius), 3, "#00ffff")
    //         this.boty2 = new Circle(this.bot.x-(Math.cos(this.anglex-(Math.PI*.5))*this.bot.radius), this.bot.y-(Math.sin(this.anglex-(Math.PI*.5))*this.bot.radius), 3, "#00ffff")
    //         this.topx2 = new Circle(this.top.x+(Math.cos(this.anglex-(Math.PI*.5))*this.top.radius), this.top.y+(Math.sin(this.anglex-(Math.PI*.5))*this.top.radius), 3, "#00ffff")
    //         this.topy2 = new Circle(this.top.x-(Math.cos(this.anglex-(Math.PI*.5))*this.top.radius), this.top.y-(Math.sin(this.anglex-(Math.PI*.5))*this.top.radius), 3, "#00ffff")
    ...
    //         this.l1 = new LineOP(this.topx, this.botx, "#FF00FF", 2)
    //         this.l2 = new LineOP(this.topy, this.boty, "#FF00FF", 2)

    //         this.l12 = new LineOP(this.topx2, this.botx2, "#FFFF00", 2)
    //         this.l22 = new LineOP(this.topy2, this.boty2, "#FFFF00", 2)
    //         this.l12.draw()
    //         this.l22.draw()
    //         this.l1.draw()
    //         this.l2.draw()
    ...
    class GeoBall {
        constructor(x, y) {
            this.body = new Circle(x, y, Math.max(80 - (linkster.difficulty * 500), 16), "transparent")
            this.body.reflect = 1
            this.body.friction = .95
            this.pattern = new Polygon(x, y, Math.max(75 - (linkster.difficulty * 500), 12), getRandomColor(), 3 + Math.round(Math.random() * 5))
            this.type = Math.floor(Math.random() * 3)
            if (this.type == 1) {
                this.pattern.color = "blue"
            } else if (this.type == 0) {
                this.pattern.color = "yellow"
            } else if (this.type == 2) {
                this.pattern.color = "red"
            }
            this.pattern.angle = Math.random() * Math.PI * 2
        }
        draw() {
            if (this.body.marked == 1) {
                if (this.marked != 1) {
                    this.marked = 1
                    linkster.score += this.pattern.sides
                }
            }
            this.body.ymom += linkster.difficulty
            this.body.frictiveMove()
            this.pattern.body.x = this.body.x
            this.pattern.body.y = this.body.y
            this.pattern.draw()
            for (let t = 0; t < balls.length; t++) {
                if (this != balls[t]) {
                    let j = 0
                    while (this.body.doesPerimeterTouch(balls[t].body)) {
                        j++
                        if (j > 300) {
                            break
                        }
                        let link = new LineOP(this.body, balls[t].body)
                        if (this.body.x > this.body.radius) {
                            this.body.x += Math.cos(link.angle()) * .07
                        } else if (this.body.x < canvas.width - this.body.radius) {
                            this.body.x += Math.cos(link.angle()) * .07
                        } else {
                            this.body.x += Math.sign((canvas.width * .5) - this.body.x)
                        }
                        this.body.y += Math.sin(link.angle()) * .07
                    }
                }
            }

            if (this.body.x < this.body.radius) {
                this.body.x += Math.sign((canvas.width * .5) - this.body.x) * 5
            } else if (this.body.x > canvas.width - this.body.radius) {
                this.body.x += Math.sign((canvas.width * .5) - this.body.x) * 5
            } else {
            }
    ...
    class Globe {
        constructor() {
            this.pointer = new Circle(0, 0, 50, "#00880044")
            this.angle = 0
            this.width = 1280
            this.radius = 49200
            this.x = 640
            this.y = 49700
            this.segs = this.radius * .8
            this.segments = []
            this.increment = (Math.PI * 2) / this.segs
            this.hill = 0
            this.ball = new Circle(640, 100, 13, "white", 0, 0, .975)
            this.rect = new Rectangle(0, 0, 1300, 750)
            for (let t = 0; t < this.segs; t++) {
                const hill = this.hilliness(this.radius)
                const point = new Point(this.x + (Math.cos(this.angle + (this.increment * t)) * hill), this.y + (Math.cos(this.angle + (this.increment * t)) * hill))
                point.hill = hill
                point.angle = this.angle + (this.increment * t)
                this.segments.push(point)
            }

            this.pointer.links = []
            for (let t = 0; t < this.segments.length; t++) {
                this.pointer.links[t] = new LineOP(this.pointer, this.segments[t])
            }
            this.links = [new LineOP(this.segments[0], this.segments[this.segments.length - 1], "green", 6)]

            for (let t = 1; t < this.segments.length; t++) {
                let link = new LineOP(this.segments[t], this.segments[t - 1], `rgb(${(t * 10) % 255}, ${255 - ((t * 10) % 255)}, 10)`, 6)
                this.links.push(link)
            }
        }
        hilliness(x) {
            x += Math.cos(this.hill / 4000) * 10
            x += Math.sin((this.hill * this.hill) / 40000) * 30
            this.hill += .01
            return x
        }
        draw() {
            ...
            for (let t = 0; t < this.segments.length; t++) {
                ...
                if (this.rect.isPointInside(this.segments[t]) && specialdiff == 0) {
                    specialdiff = this.segments[t].x - (this.x + (Math.cos(this.angle + (this.increment * t)) * this.segments[t].hill))
                }
                this.segments[t].x = this.x + (Math.cos(this.angle + (this.increment * t)) * this.segments[t].hill)
                this.segments[t].y = this.y + (Math.sin(this.angle + (this.increment * t)) * this.segments[t].hill)
                // this.segments[t].hill -= 50
                if (this.growing == 1) {
                    const length = this.pointer.links[t].hypotenuse()
                    if (length <= this.pointer.radius) {
                        if(this.links[t].hypotenuse() < 13){
                            this.segments[t].hill += ((1 - (length / this.pointer.radius))) * 4
                        }else{
                            const k1 = this.links[t].object.hill 
                            const k2 = this.links[t].target.hill 
                            this.links[t].object.hill = ((k1*3)+(k2))*.25
                            this.links[t].target.hill = ((k2*3)+(k1))*.25
                        }
                    }
            ...
            this.ball.x -= specialdiff
            this.ball.draw()
            if (this.rect.isPointInside(this.segments[0])) {
                this.links[0].draw()
            }
            for (let t = 1; t < this.segments.length; t++) {
                if (this.rect.isPointInside(this.segments[t])) {
                    this.links[t].draw()
                }
                let j = 0
                if (this.ball.isPointInside(this.segments[t])) {
                    this.ball.ymom *= -.3
                    this.ball.xmom += (this.segments[t].y - this.segments[t - 1].y) * .2
                }
                while (this.ball.isPointInside(this.segments[t])) {
                    if (j > 100) {
                        break
                    }
                    j++
    ...
    function main() {
        ...
        if (keysPressed['='] && recording == 1) {
            recording = 0
            video_recorder.stop()
            video_recorder.download('potjizya.webm')
        }
